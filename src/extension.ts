...
import { isCursorInsideJSX, isCursorInsideJSXElement } from "./isCursorInsideJSX";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.commands.registerCommand("millisecond.useMilliseconds", () => {
    ...
    let placeHolderStr =
      usedHistory.length > 2
        ? `Your searches : ${toStr(Array.from(searches))}`
        : `7 day, 45 minute, 2year, 1hr15min20sec, 1year 2week 3mon 14 hr 12 min 10sec `;
    vscode.window
      .showInputBox({
                            ...
                            if (String(usedHistory[0]).match(/^\d+$/)) {
                              ...
                              switch (fileExtension) {
                                case "js" || "ts":
                                  ...
                                  break;
                                case "jsx" || "tsx":
                                  //   let insideJsx = isCursorInsideJSX();
                                  //   console.log(insideJsx);
...
//     if (openingTagName) {
//       const closingTagMatch = text
//         .match(new RegExp(`<\\/${openingTagName}\\b[^>]*>`, "g"))
//         ?.find((match) => text.indexOf(match) > openingTagStartIndex);
//       if (closingTagMatch) {
//         const closingTagStartIndex = text.indexOf(closingTagMatch);
//         if (
//           position.isAfterOrEqual(new vscode.Position(openingTagStartIndex + 1, 0)) &&
//           position.isBefore(new vscode.Position(closingTagStartIndex, 0))
//         ) {
